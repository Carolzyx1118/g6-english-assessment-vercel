...
export async function buildPaperDraft(input: GeneratePaperDraftInput): Promise<PaperDraft> {
  ...
  const questionBlocks = splitTextIntoBlocks(rawQuestionText, input.subject, input.title || "Imported Section");
  const answerBlocks = splitTextIntoBlocks(rawAnswerText, input.subject, "Answer Key");
  const sections = buildSectionDrafts(questionBlocks, answerBlocks, sourceFiles);

  return {
    id: draftId,
    suggestedPaperId: slugify(input.title) || `paper-${draftId}`,
    title: input.title,
    ...
    nextSteps: [
      ...
      "Replace this local scaffold parser with an LLM parser when you are ready to automate full question extraction.",
    ],
