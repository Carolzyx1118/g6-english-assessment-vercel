  ...
  return (
    <div className="space-y-6">
            ...
            <div className="mt-3 flex items-end justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  {section.manualReview ? "-" : `${section.percentage}%`}
                </p>
              </div>
      ...
      {report ? (
          ...
          <div className="space-y-4">
            <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-4 space-y-4">
                {model.sections.map((section) => {
                  ...
                  return (
                    <div key={`insight-${section.sectionId}`} className="rounded-3xl bg-slate-50 px-5 py-4">
                      ...
                      <p className="mt-2 text-sm leading-6 text-slate-600">
                        {insight
                          ...
                          : (isCn
                              ? "该部分暂无单独分析，已纳入整体报告。"
                              : "This section is summarized in the overall report.")}
                      </p>
                    </div>
                  );
                })}
            ...
            {(report.studyPlan || []).length > 0 ? (
              <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div className="mt-4 space-y-4">
                  {report.studyPlan.map((stage, index) => (
                    <div key={`study-stage-${index}`} className="rounded-3xl bg-slate-50 px-5 py-4">
                      <p className="text-sm font-semibold text-slate-900">
                      ...
                      <div className="mt-3 space-y-2">
                        {(isCn ? stage.actions_cn : stage.actions_en).map((action, actionIndex) => (
                          <div key={`action-${index}-${actionIndex}`} className="rounded-2xl bg-white px-4 py-3 text-sm text-slate-700">
                            {action}
                          </div>
                        ))}
                      </div>
        ...
        <section className="rounded-[32px] border border-amber-200 bg-amber-50 px-6 py-5 text-sm text-amber-900 shadow-sm">
          {isCn
      ...
      {model.writing ? (
        <section className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-center gap-2 text-slate-900">
            ...
            <h2 className="text-xl font-bold">{isCn ? "写作分析" : "Writing Analysis"}</h2>
          </div>
          <div className="mt-5 grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
            ...
            <div className="space-y-4">
              ...
              {model.writing.evaluation?.grammarErrors?.length ? (
                <div className="rounded-3xl bg-slate-50 px-5 py-4">
                  ...
                  <div className="mt-3 space-y-3">
                    {model.writing.evaluation.grammarErrors.map((item, index) => (
                      <div key={`grammar-error-${index}`} className="rounded-2xl bg-white px-4 py-3 text-sm">
              ...
              {((isCn ? model.writing.evaluation?.suggestions_cn : model.writing.evaluation?.suggestions_en) || []).length > 0 ? (
                <div className="rounded-3xl bg-slate-50 px-5 py-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">
                    {isCn ? "写作建议" : "Writing Suggestions"}
                  </p>
                  <div className="mt-3 space-y-2">
                    {((isCn ? model.writing.evaluation?.suggestions_cn : model.writing.evaluation?.suggestions_en) || []).map((item, index) => (
                      <div key={`writing-suggestion-${index}`} className="rounded-2xl bg-white px-4 py-3 text-sm text-slate-700">
                        {item}
                      </div>
                    ))}
                  </div>
      ...
      {model.speaking.evaluation?.evaluations?.length ? (
        <section className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-center gap-2 text-slate-900">
            <Mic className="h-5 w-5 text-orange-600" />
            <h2 className="text-xl font-bold">{isCn ? "口语分析" : "Speaking Analysis"}</h2>
          </div>

          <div className="mt-4 rounded-3xl bg-orange-50 px-5 py-4 text-sm leading-7 text-orange-950">
            {isCn ? model.speaking.evaluation.overallFeedback_cn : model.speaking.evaluation.overallFeedback_en}
          </div>

          <div className="mt-5 space-y-4">
            {model.speaking.evaluation.evaluations.map((item) => (
              <div key={`${item.sectionId}-${item.questionId}`} className="rounded-[28px] border border-slate-200 bg-slate-50 p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="text-sm font-semibold text-slate-900">
                      {getSectionDisplayName(item.sectionTitle, item.sectionId, locale)} · Q{item.questionId}
                    </p>
                ...
                <div className="mt-4 grid gap-4 xl:grid-cols-2">
                  <div className="rounded-3xl bg-white px-4 py-4 text-sm leading-7 text-slate-700">
                    <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                  {[
                    ...
                    <div key={criterion.labelEn} className="rounded-3xl bg-white px-4 py-4 text-sm leading-6 text-slate-700">
                      <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">
                ...
                {((isCn ? item.suggestions_cn : item.suggestions_en) || []).length > 0 ? (
                  <div className="mt-4 rounded-3xl bg-white px-4 py-4">
                    <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">
                      {isCn ? "提升建议" : "Suggestions"}
                    </p>
                    <div className="mt-3 space-y-2">
                      {(isCn ? item.suggestions_cn : item.suggestions_en).map((suggestion, index) => (
                        <div key={`speaking-suggestion-${item.questionId}-${index}`} className="rounded-2xl bg-slate-50 px-4 py-3 text-sm text-slate-700">
                          {suggestion}
                        </div>
                      ))}
                    </div>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2 text-slate-900">
          ...
          <h2 className="text-xl font-bold">{isCn ? "逐题回看" : "Question Review"}</h2>
        </div>

        {model.sections.filter((section) => section.details.length > 0).map((section) => (
          <details
